<template>
  <section class="admin-card">
    ...
    <div v-loading="loading" class="image-resource-grid">
      <article v-for="row in rows" :key="row.id" class="image-resource-card">
        ...
        <div class="image-resource-body">
          ...
          <div class="action-row image-actions"><el-button size="small" @click="open(row)">编辑</el-button><el-button size="small" @click="copyUrl(row.url)">复制 URL</el-button><el-button size="small" type="danger" @click="remove(row)">删除</el-button></div>
        </div>
      </article>
    </div>
    ...
    <el-dialog v-model="visible" :title="form.id ? '编辑图片' : '新增图片'" width="620px" class="image-form-dialog" append-to-body>
      ...
      <template #footer><button class="btn-ghost" type="button" @click="visible = false">取消</button><button class="btn-primary" type="button" @click="save">保存</button></template>
    </el-dialog>
  </section>
</template>
...
const imageTypes = [
  { label: '站点 Logo', value: 'LOGO' },
  { label: '首页横幅', value: 'HERO' },
  { label: '全站背景', value: 'BACKGROUND' },
  { label: '文章封面', value: 'COVER' },
  { label: '头像', value: 'AVATAR' },
  { label: '推荐图', value: 'RECOMMEND' }
]
const rows = ref([])
...
const enabledBool = computed({ get: () => form.enabled === 1, set: (value) => { form.enabled = value ? 1 : 0 } })
const emptyForm = () => ({ id: null, title: '', url: '', type: query.type || 'COVER', description: '', sort: 0, enabled: 1 })
const load = async () => { loading.value = true; try { const res = await adminApi.images({ current: 1, size: 50, type: query.type || undefined }); rows.value = res.data.records || [] } catch (error) { console.error(error); ElMessage.error('图片资源加载失败') } finally { loading.value = false } }
