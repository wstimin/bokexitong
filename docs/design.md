# 系统设计文档

## 架构

系统采用前后端分离架构。Vue 3 负责前台门户、创作中心和后台管理界面，Spring Boot 3 提供 REST API。登录使用 JWT，无状态认证，后台接口通过 `ROLE_ADMIN` 保护。

## 数据模型

- `blog_user`：用户、角色、状态、头像 URL
- `article`：文章标题、摘要、封面 URL、Markdown 内容、状态、浏览/点赞/收藏统计
- `category`：文章分类
- `tag` 与 `article_tag`：标签与文章多对多关系
- `comment`：评论与回复审核
- `like_record`：点赞记录
- `favorite`：收藏记录
- `image_resource`：后台图片 URL 资源库

## 图片资源设计

系统不保存图片二进制文件，只保存外部 URL。这样部署简单，也便于接入图床、对象存储、CDN。

`image_resource.type` 的推荐取值：

- `HERO`：首页横幅
- `COVER`：文章封面
- `AVATAR`：头像
- `RECOMMEND`：推荐位图片

首页读取启用状态下排序最靠前的 `HERO` 图片作为背景。如果没有可用 URL，前端使用 CSS 兜底背景。

## 权限设计

- 游客：访问首页、文章列表、文章详情
- 登录用户：发布文章、评论、点赞、收藏、维护个人资料
- 管理员：后台仪表盘、文章管理、分类标签、图片 URL 管理、评论审核、用户封禁

## 可扩展方向

- 接入对象存储上传，将图片 URL 自动回填到 `image_resource`
- 增加文章审核流程
- 增加用户关注、专题、归档时间轴
- 将仪表盘统计改为真实 SQL 聚合趋势
- 增加 Markdown 编辑器工具栏或 WangEditor 富文本模式
