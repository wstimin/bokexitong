...
1. 先在服务器本机侧确认 `http://127.0.0.1:18080/install` 能打开。能打开说明容器正常，是 1Panel 网站反向代理没有连到这个入口。
2. 1Panel 网站反向代理地址应填写 `http://127.0.0.1:18080`。
3. 在 Compose 页面确认 `frontend`、`backend`、`mysql` 三个容器都是运行中。
4. 如果仍然 502，再确认当前 1Panel 网站使用的反向代理服务能访问宿主机 `127.0.0.1:18080`；极少数容器化反向代理需要改成宿主机网关或服务器内网 IP。
5. 如果 `127.0.0.1:18080/install` 也打不开，查看 `frontend` 容器日志；如果页面能打开但提交安装时报错，再查看 `backend` 容器日志。
