...
- 1Panel 部署：上传 `bokexitong-linux.tar.gz` 构建包后，全程在 1Panel 网页里完成容器启动、反向代理、证书和首次安装向导，数据库连接和管理员密码都在 `/install` 页面填写。
- 宝塔部署：上传 `bokexitong-linux.tar.gz` 构建包后，全程在宝塔网页里完成容器启动、反向代理、证书和首次安装向导，数据库连接和管理员密码都在 `/install` 页面填写。
...
这个包适合上传到 1Panel / 宝塔，也可以上传到 GitHub Release 供一键脚本自动下载。包内带有启动所需的 `.env`，上传后可直接启动并打开安装向导；数据库连接、站点信息和管理员密码在网页里填写。

面板上传包默认前端端口是 `18080`。容器启动后，在面板网站里把域名反向代理到 `http://127.0.0.1:18080`，申请证书后通过域名访问即可进入安装向导；如果面板反向代理访问本机回环地址出现 502，再改成服务器真实 IP 或宿主机网关地址。
